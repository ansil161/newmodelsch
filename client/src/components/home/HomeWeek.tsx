...
/* ==========================================================================
   09 - WEEKLY LIFE - the collage that builds itself
   --------------------------------------------------------------------------
   The section opens on one photograph, large and alone in the middle of a
   white stage. Under the reader's scroll it grows a little, six more prints
   slide out from behind it into a tight cluster, and the cluster then opens
   outward to the edges of the stage - leaving the sentence standing in the
   space the photographs have just vacated.

     ONE PRINT  ->  IT EXPANDS  ->  THE OTHERS EMERGE  ->  A CLUSTER
                ->  THE CLUSTER OPENS  ->  THE SENTENCE  ->  THE COLLAGE

   THE STYLESHEET DESCRIBES THE FINISHED COLLAGE. EVERYTHING ELSE IS DERIVED.

   Each print's resting place, size, proportion and tilt is real layout in
   `week.css`. That is what a reader with motion turned off gets, and what is
   left if this module never runs. The two earlier poses are measured from it
   on every refresh:

     OPEN      every print centred on the stage. The lead print is scaled to
               `.week__slot`, an invisible box carrying the opening size; the
               rest are scaled small enough to be hidden exactly behind it.
     CLUSTER   the tight collage in the middle, authored below in units of the
               opening print's height, so it is the same composition at every
               window size and is only compressed sideways when a narrow screen
               cannot hold it.

   The prints are revealed by occlusion rather than by a fade: they come out
   from behind the lead print, which is why it is on top in the stylesheet.

   THREE LAYERS, EACH OWNING DIFFERENT PROPERTIES

     .week__tile    its journey.   x / y / scale / autoAlpha / yPercent
     .week__print   its tilt.      rotation
     img            its settle.    scale

   x and y are separate tweens on different eases, so every print travels on
   a slight curve rather than a ruled line.
   ========================================================================== */

interface Print {
  photo: Photo;
  /** Resting tilt, in degrees. Small on purpose: arranged, not scattered. */
  rotate: number;
  /**
   * The tight collage before it opens: centre offset from the stage centre,
   * and height, all in units of the opening print's height.
   */
  cluster: { x: number; y: number; h: number };
  /** When it comes out from behind the lead print, as timeline fractions. */
  emerge: { at: number; duration: number };
  /** When it travels out to its place in the finished collage. */
  spread: { at: number; duration: number };
  /** yPercent of gentle drift once everything has landed. The depth. */
  drift: number;
  sizes: string;
}
...
function buildMotion(scope: HTMLElement) {
  ...
  mm.add(
    ...
    (context) => {
      ...
      const stage = scope.querySelector<HTMLElement>('.week__stage');
      const slot = scope.querySelector<HTMLElement>('.week__slot');
      const tiles = gsap.utils.toArray<HTMLElement>('.week__tile', scope);
      if (!stage || !slot || tiles.length !== PRINTS.length) return;

      const prints = tiles.map((tile) => tile.querySelector<HTMLElement>('.week__print'));
      const images = tiles.map((tile) => tile.querySelector<HTMLElement>('img'));
...
export function HomeWeek() {
  const scope = useGsapScope<HTMLElement>((_, el) => {
    ...
    const cta = el.querySelector<HTMLElement>('.week__cta');
        ...
        <ul className="week__prints" aria-label="Photographs from an ordinary school week">
          {PRINTS.map((print, i) => (
            <li className={`week__tile week__tile--${i + 1}`} key={print.photo.id}>
              <div className="week__print" style={{ '--r': `${print.rotate}deg` } as CSSProperties}>
                <Figure
                  photo={print.photo}
                  width={520}
