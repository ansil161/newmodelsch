...
export function HomeHero() {
  ...
  const scope = useGsapScope((_, el) => {
    ...
    const canvas = el.querySelector('.ch__canvas');
    ...
    const students = ['girl', 'blue', 'book']
      .map((id) => el.querySelector(`.ch__student--${id} img`))
      .filter(Boolean);
    ...
    const layers = Array.from(el.querySelectorAll('[data-depth]')).map((node) => ({
      node,
      mouse: parseFloat(node.dataset.mouse ?? '0'),
      scroll: parseFloat(node.dataset.scroll ?? '0'),
    }));
    const ground = el.querySelector('.ch__ground');
    const progress = el.querySelector('.ch__progress');
    const steps = Array.from(el.querySelectorAll('.ch__progress li'));
    ...
    /* ------------------------------------------------------ pointer + magnet */
    const cta = el.querySelector('.ch__cta');
    const arrow = cta?.querySelector('.ch__cta-arrow');
