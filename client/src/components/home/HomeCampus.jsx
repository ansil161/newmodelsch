...
function buildMotion(scope) {
  ...
  mm.add('(min-width: 900px) and (prefers-reduced-motion: no-preference)', () => {
    const slot = scope.querySelector('.hc__slot');
    const inner = scope.querySelector('.hc__inner');
    const cols = gsap.utils.toArray('.hc-col', scope);
    const panels = gsap.utils.toArray('.hc-panel', scope);
    if (!slot || !inner || cols.length !== CHAPTERS.length || panels.length !== CHAPTERS.length) {
      return;
    }

    const frame = { stack: [], hero: { x: 0, y: 0, scale: 1 } };
    ...
    const box = (el) => {
      let x = 0;
      let y = 0;
      for (let n = el; n && n !== inner; n = n.offsetParent) {
        x += n.offsetLeft;
        y += n.offsetTop;
      }
    ...
    /* ---- the captions set themselves -----------------------------------
       ...
       section is about. */
    cols.forEach((col, i) => {
      const lines = col.querySelectorAll('[data-cap]');
    ...
    cols.forEach((col, i) => {
      const img = col.querySelector('.hc-img');
      if (!img) return;
  ...
  mm.add('(max-width: 899px) and (prefers-reduced-motion: no-preference)', () => {
    ...
    cols.forEach((col, i) => {
      const frame = col.querySelector('.hc-frame');
      const img = col.querySelector('.hc-img');
      const lines = col.querySelectorAll('[data-cap]');
    ...
    const statement = scope.querySelectorAll('.hc__outro .hc__line > span');
    if (statement.length) {
      gsap.from(statement, {
        yPercent: 115,
        duration: 1,
