...
/* --------------------------------------------------------------------------
   ...
   crop focus, no srcset and no reserved ratio. `imagery.js` is where a
...
const STAGES = JOURNEY_NODES.map((node, i) => ({
  id: node.id,
...
export function HomeJourney() {
  ...
  const scope = useGsapScope((_, el) => buildMotion(el, setActive, run), []);
