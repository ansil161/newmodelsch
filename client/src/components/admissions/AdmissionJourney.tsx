...
export function AdProcess() {
  ...
  const scope = useGsapScope<HTMLElement>((_, root) => {
    const steps = gsap.utils.toArray<HTMLElement>('.aj-step', root);
    const nodes = steps.map((step) => step.querySelector<HTMLElement>('.aj-step__node'));
    const list = root.querySelector<HTMLElement>('.aj-steps');
    ...
    const film = root.querySelector<HTMLElement>('.aj__film');
    const media = root.querySelector<HTMLElement>('.aj__media');
