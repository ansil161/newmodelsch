...
const FLOATS: { photo: Photo; ratio: string; speed: number }[] = [
...
const isMainEntry = (seats: string, note: string) =>
  /entry point/i.test(seats) || /main entry/i.test(note);

export function AdEligibility() {
  const { scrollTo } = useSmoothScroll();

  const scope = useGsapScope<HTMLElement>((ctx, root) => {
    if (reduced()) return;

    /* 1 - the intro */
    const intro = root.querySelector<HTMLElement>('.wca__copy');
    ...
    const stage = root.querySelector<HTMLElement>('.wca__stage');
    const film = root.querySelector<HTMLElement>('.wca__video');
    if (film) {
    ...
    const inners = gsap.utils.toArray<HTMLElement>('.wca-float__in', root);
    gsap.from(inners, {
      opacity: 0,
      y: 20,
      scale: 0.92,
      ...
      onComplete: () => {
        ctx.add(() => {
          gsap.to(gsap.utils.toArray<HTMLElement>('.wca-float__bob', root), {
            y: '+=4',
            duration: 2.5,
            ease: 'sine.inOut',
            repeat: -1,
    ...
    mm.add('(min-width: 900px)', () => {
      gsap.utils.toArray<HTMLElement>('.wca-float', root).forEach((print) => {
        gsap.to(print, {
  ...
  const jump = (target: string) => (event: MouseEvent<HTMLAnchorElement>) => {
