...
import type { CaptchaConfig } from '@/types/api';
...
type ConfigState = { status: 'loading' } | { status: 'ready'; config: CaptchaConfig } | { status: 'error' };
...
export function LoginForm({ labelledBy }: { labelledBy: string }) {
  ...
  const [captchaNote, setCaptchaNote] = useState<string | null>(null);
