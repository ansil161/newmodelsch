...
import type { Message, Reply, Responder, Turn } from './types';
...
export interface Conversation {
  ...
  /** Ask a question that did not come from the composer - a suggestion chip. */
  send: (text: string) => void;
  /** Send whatever is in the composer, and clear it. */
  submit: () => void;
}
 ...
 */
export function useConversation(respond: Responder): Conversation {
  ...
  const send = useCallback(
    async (raw: string) => {
      ...
      const settle = (reply: Reply) => {
        const message: Message = { id: replyId, role: 'assistant', ...reply };
...
function toHistory(messages: Message[]): Turn[] {
  const turns: Turn[] = [];
