...
import type { PanelColumn, VisionPanel } from '@/types';
...
type PanelId = 'vision' | 'mission';
type Phase = 'idle' | PanelId;
...
/* ==========================================================================
   ...
   ========================================================================== */
function BrushStroke({ id, seed }: { id: string; seed: number }) {
  return (
    <svg
      className="vm__paint"
      viewBox="0 0 420 300"
...
/* ==========================================================================
   ...
   across the breakpoint swaps one build for the other cleanly.
   ========================================================================== */

interface Wiring {
  /** The pinned run, so a marker press can convert progress into a scroll. */
...
function buildPinned(root: HTMLElement, wiring: Wiring) {
  const pin = root.querySelector<HTMLElement>('.vm__pin');
  if (!pin) return;

  const brush = (id: PanelId) => root.querySelector<HTMLElement>(`[data-brush="${id}"]`);
  const panel = (id: PanelId) => root.querySelector<HTMLElement>(`[data-panel="${id}"]`);
  const marker = (id: PanelId) => root.querySelector<HTMLElement>(`[data-marker="${id}"]`);
  const cue = (id: PanelId) => root.querySelector<HTMLElement>(`[data-cue="${id}"]`);
  const label = (id: PanelId) => root.querySelector<HTMLElement>(`[data-label="${id}"]`);
  ...
  (['vision', 'mission'] as const).forEach((id, i) => {
  ...
  open('mission', 'vision', [handFrom + 4, handTo] as const, handTo);
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
function buildFlow(root: HTMLElement) {
  const blocks = gsap.utils.toArray<HTMLElement>('.vm__block', root);

  blocks.forEach((block) => {
    const paint = block.querySelector<HTMLElement>('.vm__brush');
    const id = paint?.dataset.brush as PanelId | undefined;
    if (!paint || !id) return;
    ...
    const wordmark = block.querySelector<HTMLElement>('.vm__label');
    if (wordmark) {
...
export function VisionMission() {
  ...
  const wiring = useRef<Wiring>({ trigger: null, setPhase: () => {} });
    ...
    <section ref={scope} className="vm" id="vision" aria-labelledby="vm-title">
      <div className="vm__pin">
        ...
        <div className="wrap vm__stage">
          {VISION_PANELS.map((panel: VisionPanel) => {
