...
function buildMotion(root: HTMLElement) {
  ...
  mm.add(
    ...
    (context) => {
      ...
      const stage = root.querySelector<HTMLElement>('.charter__stage');
      const steps = gsap.utils.toArray<HTMLElement>('.charter-step', root);
      const layers = gsap.utils.toArray<HTMLElement>('.charter-card__layer', root);
      const fills = gsap.utils.toArray<HTMLElement>('.charter-progress__fill', root);
      const numbers = gsap.utils.toArray<HTMLElement>('.charter-card__num-n', root);
      ...
      const parts = (step: HTMLElement) => step.querySelectorAll<HTMLElement>('[data-part]');
      const image = (layer: HTMLElement) => layer.querySelector<HTMLElement>('.charter-card__img');
      const caption = (layer: HTMLElement) =>
        layer.querySelector<HTMLElement>('.charter-card__caption');
...
export function CharterScrollSection() {
  ...
  const scope = useGsapScope<HTMLElement>((_ctx, root) => {
    ...
    rise(root.querySelectorAll<HTMLElement>('[data-intro]'), { trigger: root, y: 16, delay: 0.2 });
...
function StepIndicator({ entry }: { entry: CharterEntry }) {
  return (
...
function CharterHeading({ entry }: { entry: CharterEntry }) {
  return (
...
function CharterDescription({ entry }: { entry: CharterEntry }) {
  return (
    <>
...
function CardContent({ entry, index }: { entry: CharterEntry; index: number }) {
  return (
    <figure className="charter-card__layer" style={{ zIndex: index + 1 } as CSSProperties}>
      <img
