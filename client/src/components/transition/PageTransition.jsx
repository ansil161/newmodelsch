...
/* ==========================================================================
   ...
   `lib/stage.js`. Entrances mount and build themselves while the sheet is
 ...
 */
function dims(lockup) {
  const scale = gsap.getProperty(lockup, 'scaleX') || 1;
...
export function PageTransition() {
  ...
  const parts = useCallback(() => {
    ...
    if (!el) return null;
    const sheet = el.querySelector('.pt__sheet');
    const stage = el.querySelector('.pt__stage');
    const lockup = el.querySelector('.pt__lockup');
   ...
   */
  const plot = useCallback((lockup, share) => {
    const brand = brandEl();
    if (!brand) return null;
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const hero = loaded(document.querySelector('.ch__student--girl img, .hero__frame img, .cover__media img, .ach__frame img'));
