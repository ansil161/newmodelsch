...
export function Footer() {
  ...
  const scope = useGsapScope<HTMLElement>((_, el) => {
    const top = el.querySelector<HTMLElement>('.foot__top');
    const dir = el.querySelector<HTMLElement>('.foot__dir');
    const brand = el.querySelector<HTMLElement>('.foot__brand');
    const wordmark = el.querySelector<HTMLElement>('.foot__wordmark');
    const title = el.querySelector<HTMLElement>('.foot__title');
    const cta = el.querySelector<HTMLElement>('.foot__cta');
    ...
    if (!reduced()) {
      el.querySelectorAll<HTMLElement>('.foot__rule').forEach((rule) => {
        gsap.from(rule, {
          scaleX: 0,
          duration: 1.4,
          ease: 'power3.inOut',
 ...
 * itself.
 */
function fitWordmark(el: HTMLElement) {
