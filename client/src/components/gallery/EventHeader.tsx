...
export function EventHeader({ year, event }: { year: GalleryYear; event: GalleryEvent }) {
  const open = useLightbox();

  const scope = useGsapScope<HTMLElement>((_, el) => {
    if (reduced()) return;
    const title = el.querySelector<HTMLElement>('.gal-ehead__title');
...
export function EventPager({ year, event }: { year: GalleryYear; event: GalleryEvent }) {
