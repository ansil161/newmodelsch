...
export function GalleryCta() {
  const scope = useGsapScope<HTMLElement>((_, el) => {
    const title = el.querySelector<HTMLElement>('.gal-cta__title');
    const revert = title ? lines(title, { trigger: el }) : undefined;
    draw(el, { trigger: el, delay: 0.7 });
    rise(el.querySelectorAll<HTMLElement>('.gal-cta__tag, .gal-cta__lead, .gal-cta__actions, .gal-cta__years'), {
      trigger: el,
      y: 20,
      delay: 0.3,
    });
    ...
    const button = el.querySelector<HTMLElement>('.gal-cta__btn');
