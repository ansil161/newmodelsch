...
import type { GalleryEvent, GalleryYear } from '@/constants/gallery';
import { useGsapScope } from '@/hooks/useGsapScope';
import { rise, unmask } from '@/lib/motion';
import { Icon } from '@/components/common/Icon';
import { Figure } from '@/components/editorial';
import type { FigureRatio } from '@/components/editorial';

/* ==========================================================================
   EVENT GRID - a year, event by event
   --------------------------------------------------------------------------
   Laid out in rows whose widths change as the year goes on - a wide card and
   a tall one, then three, then a tall one and a wide one - so a page of ten
   events reads as a spread rather than a dashboard of equal tiles.

   A short final row never leaves a gap: one event left over takes the whole
   width, two share it.
   ========================================================================== */

type Variant = 'full' | 'feature' | 'tall' | 'third';
...
const REMAINDER: Record<number, number[]> = { 1: [12], 2: [7, 5] };

const VARIANT: Record<number, { variant: Variant; ratio: FigureRatio; width: number; sizes: string }> = {
...
function layout(events: GalleryEvent[]) {
  const rows: { event: GalleryEvent; span: number }[][] = [];
  let i = 0;
  let r = 0;
...
export function EventGrid({ year }: { year: GalleryYear }) {
  const scope = useGsapScope<HTMLElement>((_, el) => {
    el.querySelectorAll<HTMLElement>('.gal-ev').forEach((card) => {
      unmask(card.querySelector<HTMLElement>('.gal-ev__media .fig'), { trigger: card });
      rise(card.querySelectorAll<HTMLElement>('.gal-ev__text > *'), { trigger: card, y: 18, delay: 0.25, stagger: 0.06 });
    });
    rise(el.querySelectorAll<HTMLElement>('.gal-evs__head > *'), { y: 20 });
  }, [year.id]);
...
export function YearPager({ year }: { year: GalleryYear }) {
  ...
  const scope = useGsapScope<HTMLElement>((_, el) => {
    rise(el.querySelectorAll<HTMLElement>('.gal-pager__item'), { y: 30, stagger: 0.1 });
  }, [year.id]);
