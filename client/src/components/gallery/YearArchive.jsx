...
export function YearArchive() {
  const scope = useGsapScope((_, el) => {
    const track = el.querySelector('.gal-years__track');
    if (!track) return;
    rise(el.querySelectorAll('.gal-year'), { trigger: track, y: 64, stagger: 0.12 });
    unmask(el.querySelectorAll('.gal-year__cover'), { trigger: track, stagger: 0.12 });
    grow(el.querySelector('.gal-years__line-fill'), {
      axis: 'x',
      trigger: track,
      end: 'bottom 60%',
    });
    rise(el.querySelectorAll('.gal-year__node'), {
      trigger: track,
      y: 0,
      delay: 0.4,
      stagger: 0.14,
...
export function YearCard({ year, index }) {
