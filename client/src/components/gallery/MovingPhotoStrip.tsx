...
export function MovingPhotoStrip() {
  ...
  const scope = useGsapScope<HTMLElement>((_, el) => {
    const viewport = el.querySelector<HTMLElement>('.gal-strip__viewport');
    const track = el.querySelector<HTMLElement>('.gal-strip__track');
    if (!viewport || !track) return;

    rise(el.querySelectorAll<HTMLElement>('.gal-strip__item'), {
      trigger: viewport,
      y: 48,
      stagger: 0.04,
    });
    ...
    const onEnter = (e: PointerEvent) => e.pointerType === 'mouse' && slow();
    const onLeave = (e: PointerEvent) => e.pointerType === 'mouse' && resume();
    const onFocusIn = (e: FocusEvent) => {
      slow();
      const item = (e.target as HTMLElement).closest<HTMLElement>('.gal-strip__item');
      if (item) {
    ...
    const onFocusOut = (e: FocusEvent) => {
      if (!viewport.contains(e.relatedTarget as Node | null)) resume();
    };
