...
export function FeaturedMemory() {
  ...
  const scope = useGsapScope<HTMLElement>((_, el) => {
    rootRef.current = el;
    const body = el.querySelector<HTMLElement>('.gal-feat__body');
    unmask(el.querySelector<HTMLElement>('.gal-feat__visual'), { trigger: body, from: 'left' });
    rise(el.querySelectorAll<HTMLElement>('.gal-feat__pick'), { trigger: body, y: 16, stagger: 0.06 });
    rise(el.querySelectorAll<HTMLElement>('.gal-feat__text > *'), { trigger: body, y: 22, delay: 0.25 });
    rise(el.querySelectorAll<HTMLElement>('.gal-feat__print'), { trigger: body, y: 50, delay: 0.45, stagger: 0.1 });
    drift(el.querySelector<HTMLElement>('.gal-feat__stamp'), -70, { trigger: el });
  }, []);
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const main = el.querySelector<HTMLElement>(`.gal-feat__main[data-index="${active}"]`);
