...
const METHOD_LABELS = { dense: 'semantic', sparse: 'keyword' };
 ...
 */
export function TraceView({ trace, knowledgeBaseId }) {
  const facts = [
...
function KeyValues({ title, data }) {
  ...
  return (
    <div className="kb-kv">
      ...
      {entries.length ? (
        <Facts items={entries.map(([key, value]) => [humanize(key), display(value)])} />
      ) : (
...
function HitsTable({ hits, knowledgeBaseId, caption }) {
  return (
