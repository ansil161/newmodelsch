...
import type { Chunk, DocumentVersion } from '@/types/knowledgeBase';
 ...
 */
export function ChunkBrowser({ documentId, versions, activeVersion }: { documentId: string; versions: DocumentVersion[]; activeVersion: number | null }) {
  const [version, setVersion] = useState<number | undefined>(activeVersion ?? versions[0]?.number);
...
function ChunkItem({ chunk, search, open, onToggle }: { chunk: Chunk; search: string; open: boolean; onToggle: () => void }) {
