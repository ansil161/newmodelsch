...
export function EmptyState({ icon = 'document', title, children, action }: { icon?: string; title: string; children?: ReactNode; action?: ReactNode }) {
  return (
...
const ALERT_ICONS: Record<AlertTone, string> = { error: 'alert', warn: 'alert', info: 'info', success: 'check' };
...
export function LoadError({ error, onRetry, title = 'This could not be loaded' }: { error: ApiError; onRetry?: () => void; title?: string }) {
  return (
    <Alert
      tone="error"
      title={title}
