...
/* --------------------------------------------------------------------------
   ...
   one, rather than by the alt text in `imagery.ts` - several entries there
 ...
 */
const PERIOD_AT: Record<number, string> = {
  0: 'Morning',
  4: 'Midday',
  6: 'Afternoon',
};
...
const EVENTS: DayEvent[] = DAY_BLOCKS.map((block, i) => ({
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
const THREAD: { d: string; w: number; o: number }[] = [
...
export function SlDay() {
  const scope = useGsapScope<HTMLElement>((_, el) => {
    const head = el.querySelector<HTMLElement>('.day__title');
    if (head) lines(head, { trigger: el, stagger: 0.09 });
    rise(el.querySelectorAll<HTMLElement>('[data-intro]'), { trigger: el, y: 18, delay: 0.3 });
    ...
    el.querySelectorAll<HTMLElement>('.day-row').forEach((row) => {
      rise(row.querySelectorAll<HTMLElement>('[data-row]'), {
        trigger: row,
        y: 22,
        stagger: 0.08,
      });

      const strokes = row.querySelectorAll<SVGPathElement>('.day-thread__stroke');
      if (strokes.length) grow(strokes, { trigger: row, end: 'bottom 78%' });

      const frame = row.querySelector<HTMLElement>('.day-row__figure');
