...
export function SlCampus() {
  ...
  const scope = useGsapScope((_, el) => {
    unmask(el.querySelectorAll('.campex__frame'), { trigger: el, from: 'bottom' });
    rise(el.querySelectorAll('.campex__row'), { trigger: el, y: 16, stagger: 0.05 });
...
export function SlBeyond() {
  const scope = useGsapScope((_, el) => {
    const strands = el.querySelectorAll('.strand');
    strands.forEach((strand, i) => {
      unmask(strand.querySelectorAll('.fig'), {
        trigger: strand,
        from: i % 2 === 0 ? 'left' : 'right',
      });
      rise(strand.querySelectorAll('[data-lift]'), {
        trigger: strand,
        y: 22,
        stagger: 0.06,
      });
  ...
  return (
    <HorizontalGallery
      ...
      frames={galleryImages.map((photo, i) => ({
        photo,
        size: (['tall', 'md', 'sm', 'lg', 'md', 'tall', 'sm', 'md', 'lg', 'sm'])[i % 10],
        shape: (['arch', 'frame', 'round', 'frame', 'blob', 'arch', 'round', 'blob', 'frame', 'arch'])[i % 10],
      }))}
    >
      <SectionHead
        sticker="This term"
...
/* --------------------------------------------------------------------------
   ...
   than inside it. See `SlVoices.jsx`.
   -------------------------------------------------------------------------- */
...
/* --------------------------------------------------------------------------
   ...
   rather than inside it - the same arrangement as the day. See `SlSafety.jsx`.
   -------------------------------------------------------------------------- */
