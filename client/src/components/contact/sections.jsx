...
export function CoHello() {
  const scope = useGsapScope((_, el) => {
    const head = el.querySelector('.hello__title');
    if (head) lines(head, { trigger: el, start: 'top 90%' });
    draw(el, { trigger: el, delay: 0.55, start: 'top 90%' });
    rise(el.querySelectorAll('.hello__body > *'), {
      trigger: el,
      start: 'top 90%',
      delay: 0.3,
      stagger: 0.07,
    });
    unmask(el.querySelectorAll('.hello__fig'), {
      trigger: el,
      start: 'top 90%',
      from: 'bottom',
    });
...
export function CoChannels() {
  const scope = useGsapScope((_, el) => {
    rise(el.querySelectorAll('.chan'), { trigger: el, y: 20, stagger: 0.07 });
    rise(el.querySelectorAll('.hours__row'), {
...
export function CoDirections() {
  const scope = useGsapScope((_, el) => {
    rise(el.querySelectorAll('.route'), { trigger: el, y: 18, stagger: 0.07 });
    unmask(el.querySelectorAll('.dir__fig'), { trigger: el, from: 'bottom' });
  }, []);
