...
const tiltOf = (el) => parseFloat(getComputedStyle(el).rotate) || 0;
...
function collect(root) {
  ...
  const groups = all(root, '[data-bm-exp]').flatMap((el) => {
    ...
    const link = one(root, `[data-bm-link="${id}"]`);
    ...
    return [
      {
        ...
        thread: all(el, '[data-bm-thread]'),
        link,
        lines: link ? all(link, '[data-bm-draw]') : [],
        dots: link ? all(link, '[data-bm-dot]') : [],
  ...
  return {
    ...
    swash: all(root, '.bm__swash [data-stroke]'),
    ring: one(root, '[data-bm-ring]'),
    ringPaths: all(root, '[data-bm-ring] path'),
    spokes: all(root, '[data-bm-spokes] path'),
    words: all(root, '[data-bm-word]'),
    map: one(root, '[data-bm-map]'),
    orbit: one(root, '[data-bm-orbit]'),
    specks: all(root, '[data-bm-speck]'),
    decor: one(root, '[data-bm-decor]'),
    marks: all(root, '.bm-mark'),
    notes: all(root, '[data-bm-note]'),
    pen: all(root, '.bm-note__arrow [data-stroke], .bm-mark [data-stroke]'),
...
function hover(r, track, pointer) {
  const pulse = track('bmPulse', (g) => {
    gsap.fromTo(
      g.dots,
      { scale: 1 },
  ...
  const render = track('bmRender', (g) => {
    const { photo: onPhoto, card: onCard } = g.hover;
    const on = onPhoto || onCard;
    const settle = { duration: 0.5, ease: 'power3.out', overwrite: 'auto' };
...
function activate(r, track) {
  let lit = -1;

  const light = track('bmLight', (index) => {
    if (index === lit) return;
    lit = index;
    r.groups.forEach((g, i) => {
      const on = i === index;
...
export function animateBeyond(root) {
  ...
  mm.add(
    ...
    (context) => {
      ...
      const track = (name, fn) => context.add(name, fn);
      ...
      const state = { ready: false, inView: false };
      const offs = [];
