...
const TALL: Photo = everydayImages.classroom;
const NEAR: Photo = studentImages[6];
...
export function AcProof() {
  const scope = useGsapScope<HTMLElement>((_ctx, root) => {
    const body = root.querySelector<HTMLElement>('.ac-proof__body');
    const stats = root.querySelector<HTMLElement>('.ac-proof__stats');
    const rule = root.querySelector<HTMLElement>('.ac-proof__rule');

    /* ------------------------------------------------------------------
       Arrival
       ------------------------------------------------------------------ */
    const title = root.querySelector<HTMLElement>('.ac-proof__title');
    if (title) lines(title, { trigger: root, start: 'top 78%' });

    if (body) {
      rise(body.querySelectorAll<HTMLElement>('.ac-proof__frame'), {
        trigger: body,
        start: 'top 82%',
        y: 40,
        stagger: 0.12,
      });
      rise(body.querySelectorAll<HTMLElement>('[data-proof-in]'), {
        trigger: body,
        start: 'top 82%',
        y: 22,
        stagger: 0.08,
    ...
    if (stats) {
      rise(stats.querySelectorAll<HTMLElement>('.ac-proof__stat'), {
        trigger: stats,
        start: 'top 86%',
        y: 18,
        stagger: 0.12,
      ...
      /* The figures count from zero, in reading order, and land exactly on
         ...
         whole time. */
      count(stats.querySelectorAll<HTMLElement>('.ac-proof__num'), {
        trigger: stats,
        start: 'top 86%',
        delay: 0.18,
        stagger: 0.14,
    ...
    mm.add(DESYNC, () => {
      const near = root.querySelector<HTMLElement>('.ac-proof__shot--near');
      const tall = root.querySelector<HTMLElement>('.ac-proof__shot--tall');
      const text = root.querySelector<HTMLElement>('.ac-proof__text');
      ...
      const clamp = (max: number, n: number) => gsap.utils.clamp(-max, max, n);
