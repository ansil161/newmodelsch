...
export function CurriculumMap({ stages, colophon, id }) {
  ...
  const scope = useGsapScope(
    (_ctx, root) => {
      const title = root.querySelector('.cm__title');
      if (title) lines(title, { trigger: root });
      rise(root.querySelectorAll('[data-intro]'), {
        trigger: root,
        delay: 0.2,
        stagger: 0.07,
      });

      const pin = root.querySelector('.cm__pin');
      const fill = root.querySelector('.cm-nav__fill');
      if (!pin || !fill) return;

      rise(pin.querySelectorAll('[data-enter]'), {
        trigger: pin,
        y: 22,
        stagger: 0.08,
      });
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const plates = root.querySelectorAll('.cm-plate');
    const panels = root.querySelectorAll('.cm-panel');
    ...
    const main = inPlate.querySelector('.cm-shot--main');
    const second = inPlate.querySelector('.cm-shot--second');
      ...
      <div className="cm__pin">
        <div className="wrap cm__frame">
          ...
          <nav
            ...
            data-enter
            style={{ '--i': active, '--fill': (active + 1) / count }}
          >
