...
import type { HandKind, HandTone } from '@/components/editorial';
...
/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   A learning map pasted up on paper rather than a list of skills. Four
   printed photographs sit around one hand-drawn thought node, each with a
   small cream card half over its corner, and pencil lines run from the node
   out to every one of them.

     copy, top left     eyebrow, a two-line serif headline, a short lead
     the node           "Beyond the Syllabus" in an imperfect ring, with a
                        dashed orbit round it
     four experiences   Collaboration top left, Communication top right,
                        Problem Solving bottom left, Creativity bottom right
     the pen            handwritten notes, arrows, stars, dots and crosses

   ONE DRAWING, THREE LAYOUTS

   From 1180px the stage is a size container, 100cqi wide and 66cqi tall, and
   everything on it is placed in those units from the `layout` below - so the
   composition scales as one sheet. Between 760px and 1180px the four sit in
   a 2 x 2 grid with the node pinned at its centre and four short spokes
   pointing at them. Below 760px the map becomes a story read downwards: copy,
   node, then each experience hanging off a pencil thread.

   Motion lives in `beyond-motion.ts` and reads only data attributes, so the
   markup here can change shape without the animation having to follow it.
   ========================================================================== */

type Tone = 'sun' | 'blue' | 'green' | 'coral';

/** A place on the wide stage: `x`, `y` and `w` in stage units, `r` in degrees. */
interface Spot {
  x: number;
  y: number;
  w: number;
...
interface Experience {
  ...
  note: string;
  layout: {
    /** `ratio` is width over height; `shape` is the print's uneven corners. */
    photo: Spot & { ratio: number; shape: string };
    card: Spot;
    /** The arrow is placed relative to the note, not to the stage. */
    note: Omit<Spot, 'w'> & { arrow: Spot & { flip?: boolean } };
  };
  /** The pencil line from the node, in stage units. `ghost` is the second,
   *  lighter pass a hand makes; `mid` sits on the orbit, `end` at the print. */
  link: { d: string; ghost: string; mid: [number, number]; end: [number, number] };
  /** Resting sway of the print, in degrees, from and to. */
  float: [number, number];
  /** How far the print slides against the scroll, in percent of its height. */
  drift: number;
}
...
const MARKS: { kind: HandKind; tone: HandTone; x: number; y: number; w: number; r: number }[] = [
...
const place = (key: string, { x, y, w, r }: Spot) => ({
  [`--${key}x`]: x,
  [`--${key}y`]: y,
  [`--${key}w`]: w,
  [`--${key}r`]: `${r}deg`,
...
export function AcBeyond() {
  const scope = useGsapScope<HTMLElement>((_, el) => animateBeyond(el), []);
    ...
    >
      ...
      <p className="bm-note" aria-hidden="true" data-bm-note>
        ...
        <Hand
          ...
          style={place('a', note.arrow) as CSSProperties}
        />
      </p>
    ...
    <div className="bm__decor" aria-hidden="true" data-bm-decor>
      {MARKS.map((mark) => (
        <Hand
          ...
          tone={mark.tone}
          className="bm-mark"
          style={place('d', mark) as CSSProperties}
      ...
      <p
        ...
        style={{ '--nx': 45.2, '--ny': 49, '--nr': '-3deg' } as CSSProperties}
        data-bm-note
      >
        ...
        <Hand
          ...
          flip
          className="bm-note__arrow"
          style={place('a', { x: -2.6, y: -3.4, w: 2.8, r: 58 }) as CSSProperties}
        />
      </p>
    </div>
  );
