...
/* ==========================================================================
   02b - NEXT STEPS (after Class 10)
   --------------------------------------------------------------------------
   The school ends at Class 10, so the last thing it owes a family is a clear
   view of what comes next. Set as an art-directed spread rather than a list:

     copy, top left       eyebrow, serif headline with one written word,
                          a short lead, and a quiet round CTA
     the portrait         one large circular photograph, a pale blob behind
                          it, thin orbit rings, nodes and micro-labels around
                          it, and the yellow "Next Step" badge over its foot
     four paths           floating panels at 1, 7, 4 and 6 o'clock around the
                          portrait, each with its own circular photograph
     the page edge        a green blob top right, a yellow one bottom left, a
                          line-drawn sprig bottom right

   THREE LAYERS, FOR WHOEVER ANIMATES IT

   Nothing here moves yet. The markup is split so a GSAP pass can take each
   layer on its own without restructuring anything:

     [data-nx-shape]        background blobs and the sprig     - drift
     [data-nx-orbit]        the ring SVG                       - rotate
     [data-nx-ring]         individual rings                   - rotate / draw
     [data-nx-node]         the dots on the rings              - travel
     [data-nx-card]         the path panels, with data-from    - reveal / parallax
     [data-nx-hero]         the portrait                       - scale / unmask
     [data-nx-badge]        the badge                          - settle
     [data-nx-lines]        the headline                       - lines()

   On a wide screen the stage is a size container and every position is in
   `cqi`, so the composition keeps its proportions from 1180px up. Below
   that it is a readable column: copy, portrait, then the four panels.
   ========================================================================== */

type Tone = 'lav' | 'sun' | 'green' | 'blue';
...
interface NextPath {
  ...
  tone: Tone;
  photo: Photo;
  /** The side a reveal should enter from. */
...
export function AcNext() {
  ...
  const toPaths = (event: MouseEvent<HTMLAnchorElement>) => {
...
function PathCard({ path, slot }: { path: NextPath; slot: number }) {
  return (
    <li
