...
type El = gsap.DOMTarget;

interface Common {
  /** What the trigger watches. Defaults to the animated element itself. */
  trigger?: Element | null;
  start?: string;
  delay?: number;
  /** Play once and forget, which is the default. `false` replays on re-entry. */
  once?: boolean;
}
...
/* --------------------------------------------------------------------------
   ...
   forty fragments.
   -------------------------------------------------------------------------- */
export function lines(target: El, opts: Common & { stagger?: number } = {}) {
  if (reduced()) return;
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
export function rise(targets: El, opts: Common & { y?: number; stagger?: number } = {}) {
  if (reduced()) return;
...
export function unmask(
  figures: El,
  opts: Common & { from?: 'bottom' | 'top' | 'left' | 'right'; stagger?: number } = {},
) {
  const els = gsap.utils.toArray<HTMLElement>(figures);
...
/* --------------------------------------------------------------------------
   ...
   layers.
   -------------------------------------------------------------------------- */
export function drift(target: El, amount = 80, opts: { trigger?: Element | null } = {}) {
  if (reduced()) return;
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
export function draw(scope: Element, opts: Common = {}) {
  if (reduced()) return;
  const marks = scope.querySelectorAll<HTMLElement>('.mark, .mark-ul, .mark-ring');
...
export function grow(
  target: El,
  opts: { trigger?: Element | null; axis?: 'x' | 'y'; svg?: boolean; end?: string } = {},
) {
  const els = gsap.utils.toArray<HTMLElement | SVGPathElement>(target);
  if (!els.length) return;
...
export function rail(
  track: HTMLElement,
  section: HTMLElement,
  opts: { end?: (distance: number) => string; onProgress?: (p: number) => void } = {},
) {
...
export function stand(
  targets: El,
  opts: { trigger?: Element | null; start?: string; end?: string } = {},
) {
  if (reduced()) return;
  const els = gsap.utils.toArray<HTMLElement>(targets);
  if (!els.length) return;
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
export function settle(el: HTMLElement, strength = 0.32) {
