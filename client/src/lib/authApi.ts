import type { ApiUser, AuthUser, CaptchaConfig, CaptchaProvider, LoginCredentials } from '@/types/api';
...
const toUser = (user: ApiUser): AuthUser => ({
  id: user.id,
  email: user.email,
...
export const authApi = {
  ...
  async loginConfig(): Promise<CaptchaConfig> {
  ...
  async login({ email, password, captchaToken }: LoginCredentials): Promise<AuthUser> {
    const data = await api.post<{ user: ApiUser }>(
      '/auth/login/',
  ...
  async me(signal?: AbortSignal): Promise<AuthUser> {
    const data = await api.get<{ user: ApiUser }>('/auth/me/', { signal });
    return toUser(data.user);
  },
};
