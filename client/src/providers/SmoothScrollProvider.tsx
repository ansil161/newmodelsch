...
interface ScrollToOptions {
  /** Jump rather than animate — used on route changes, where a 1.6s glide
   *  through the outgoing page's content is not a transition, it is a delay. */
  immediate?: boolean;
}

interface SmoothScrollValue {
  /**
   * Scroll to an element or offset. Falls back to native scroll when Lenis is
   * off. Always prefer this over `window.scrollTo`: Lenis owns the scroll
   * position, and a native jump leaves both Lenis and ScrollTrigger holding a
   * stale value — which silently strands one-shot reveals in their from-state.
   */
  scrollTo: (
