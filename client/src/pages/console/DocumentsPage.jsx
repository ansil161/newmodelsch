...
import { useDocumentActions } from '@/components/knowledge-base/useDocumentActions';
...
const PAGE_SIZE = 20;
const COLUMNS = 7;
const DEFAULTS = { q: '', status: '', type: '', source: '', sort: '-updated_at' };
...
export function DocumentsPage() {
  ...
  const resource = useResource(
    (signal) =>
      knowledgeBaseApi.documents(
        knowledgeBase.id,
        { search: q, status, type, source: source, ordering: sort, page, pageSize: PAGE_SIZE },
        signal,
      ),
  ...
  const sortBy = (ordering) => update({ sort: ordering });
