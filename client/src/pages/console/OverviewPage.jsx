...
export function OverviewPage() {
  ...
  const byType = Object.entries(overview.sources.byType)
    .filter(([, count]) => count)
    .map(([type, count]) => `${formatCount(count ?? 0)} ${SOURCE_TYPE_LABELS[type].toLowerCase()}`)
    .join(' · ');
...
function GetStarted({ knowledgeBaseId, canEdit }) {
  return (
...
function QueriesPanel({ overview, knowledgeBaseId }) {
...
function ActivityItem({ entry, knowledgeBaseId }) {
