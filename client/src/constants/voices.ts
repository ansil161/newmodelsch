import type { VoiceCategory, VoiceStory } from '@/types';
...
/* ==========================================================================
   ...
   NOT EXPORTED FROM `constants/index.ts`. This module imports from it, so
...
function need<T>(list: readonly T[], match: (item: T) => boolean, what: string): T {
  const found = list.find(match);
  if (!found) throw new Error(`voices.ts: ${what} is no longer in its source list.`);
  return found;
}

const said = (id: string) => need(TESTIMONIALS, (t) => t.id === id, `testimonial ${id}`);
const filmed = (id: string) => need(STUDENT_FILMS, (f) => f.id === id, `student film ${id}`);
const alum = (id: string) => need(ALUMNI, (a) => a.id === id, `alumni profile ${id}`);
const staff = (id: string) => need(LEADERSHIP, (p) => p.id === id, `leadership profile ${id}`);
...
const ALUMNI_VOICES: VoiceStory[] = [
  {
    id: 'praveen',
    name: said('t5').name,
    role: said('t5').role,
