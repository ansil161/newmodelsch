/* ==========================================================================
   GALLERY - the living yearbook's data
   --------------------------------------------------------------------------
   Every photograph, category, academic year and event on the Gallery pages
   is declared in this file and nowhere else. No gallery component holds an
   image, a count or a date of its own.

   ADDING THE SCHOOL'S OWN PHOTOGRAPHS

     1. Put the files anywhere under `client/public`, for example
        `public/images/gallery/2025-26/cultural-fest/01.jpg`.
     2. Add an entry to `LIBRARY` whose `id` is that path:
          fest01: { id: '/images/gallery/2025-26/cultural-fest/01.jpg',
                    alt: 'What the photograph shows', ratio: 1.5 },
     3. Reference it from an event's `photos`, a category, or both.

   `ratio` is width divided by height. It is optional, but set it: the event
   pages reserve each photograph's real shape before the file arrives and lay
   the columns out by it, so a missing ratio falls back to 4:3 and the column
   heights will be approximate.

   Counts are never typed in. Every "14 photographs" and "8 events" on the
   site is derived from the arrays below, so the numbers cannot drift from
   what a visitor can actually open.

   THE CURRENT PHOTOGRAPHS ARE STAND-INS.
   They are licensed stock frames chosen to match each event, and several
   appear in more than one event. Replace them event by event as the school's
   photography is delivered.
   ========================================================================== */

import type { Photo } from './imagery';
...
export interface GalleryPhoto extends Photo {
  ...
  ratio?: number;
  /** A short human caption, used on the moving strip and in the viewer. */
  caption?: string;
}
...
export interface GalleryEvent {
  id: string;
  title: string;
  /** As printed: '24 September 2025'. */
  date: string;
  venue?: string;
  category: GalleryCategoryId;
  /** One sentence, shown on cards. */
  description: string;
  /** A short paragraph, shown on the event's own page. */
  story: string;
  cover: GalleryPhoto;
  photos: GalleryPhoto[];
}
...
export interface GalleryYear {
  ...
  subtitle: string;
  intro: string;
  /** Span of the academic year, as printed. */
  span: string;
  cover: GalleryPhoto;
  events: GalleryEvent[];
}
...
export interface GalleryCategory {
  ...
  title: string;
  lead: string;
  /** Written beside the composition in the hand face. */
  note: string;
  photos: GalleryPhoto[];
}
...
const L: Record<keyof typeof LIBRARY, GalleryPhoto> = LIBRARY;
...
const event = (e: Omit<GalleryEvent, 'photos'> & { photos: GalleryPhoto[] }): GalleryEvent => ({
  ...e,
...
export const yearPhotoCount = (year: GalleryYear) =>
...
export const findYear = (yearId?: string) => GALLERY_YEARS.find((y) => y.id === yearId);

export const findEvent = (yearId?: string, eventId?: string) =>
  findYear(yearId)?.events.find((e) => e.id === eventId);

export const yearHref = (year: GalleryYear) => `${ROUTES.gallery}/${year.id}`;
export const eventHref = (year: GalleryYear, ev: GalleryEvent) =>
...
export const GLIMPSE_PHOTOS: (GalleryPhoto & { label: string })[] = [
...
export const MOMENT_PHOTOS: GalleryPhoto[] = [
...
export const FEATURED_STORIES = [
  ...
  .filter((s): s is { year: GalleryYear; event: GalleryEvent } => s !== null);
